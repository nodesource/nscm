...
function checkPackages (opts, callback) {
  ...
  mapLimit(packages, opts.concurrency || 15, function (payload, next) {
    ...
    request({
      ...
      debug(payload.name, payload.version, res.statusCode, JSON.stringify(res.headers, null, 2), body)

      if (res.statusCode === 401) {
        return next(new Error(res.headers['npm-notice'] || 'authentication error, please run `nscm signin` or set a correct token'))
      }
...
function flatten (list) {
  ...
  function crawl (deps) {
    Object.keys(deps.dependencies).forEach(function (pkg) {
      if (!added[pkg + deps.dependencies[pkg].version]) {
        added[pkg + deps.dependencies[pkg].version] = true
        result.push({
          name: pkg,
