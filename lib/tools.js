...
function getPackages (options, callback) {
  ...
  cpr(path.join(options.cwd || '.', 'package.json'), path.join(tmpDir, 'package.json'), {
    overwrite: true
  }, execute)
...
function checkPackages (opts, callback) {
  ...
  async.mapLimit(packages, opts.concurrency || 15, function (payload, next) {
    debug(`checking score for ${payload.name}@${payload.version}`)
    request({
      url: url.resolve(opts.publicRegistry, `/api/v1/package?name=${payload.name}&version=${payload.version}`),
      method: 'GET',
      headers: {
